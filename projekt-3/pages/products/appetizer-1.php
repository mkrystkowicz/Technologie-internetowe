<!DOCTYPE html>
<html lang="pl">

<head>
  <meta charset="UTF-8" />
  <meta http-equiv="X-UA-Compatible" content="IE=edge" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <link rel="stylesheet" href="../../style/style.css" />
  <link rel="stylesheet" href="../../style/product.css" />
  <link rel="preconnect" href="https://fonts.googleapis.com" />
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
  <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;500&display=swap" rel="stylesheet" />
  <title>Schabowe Babuni</title>
</head>

<body>
  <header>
    <div class="container">
      <a href="/index.php"><img src="/assets/logo.png" alt="logo" class="logo" /></a>
      <div class="slogan">
        <q>Z naszym jedzeniem poczujesz sie jak w domu.</q>
      </div>
    </div>
  </header>
  <main>
    <div class="container">
      <div class="navigation-btn-container">
        <a href="../appetizer.php"><button class="btn">
            < Powrót</button></a>
      </div>
      <div class="product-container">
        <img src="/assets/product-images/kanapka-ze-smalcem.jpeg" alt="kanapka-ze-smalcem" />
        <p class="desc">Przepyszna kanapka ze smalcem oraz ogórkiem kiszonym sprawi, że nabierzesz apetytu.</p>
        <button class="btn">Zamów</button>
      </div>
    </div>
  </main>
  <footer>
    <div class="container">
      <div class="col contact">
        <p class="title">Dane kontaktowe:</p>
        <p>Al. Ujazdowskie 11</p>
        <p>00-950 Warszawa</p>
        <p>tel. 22 52 12 888</p>
        <p>email: kontakt@schabowebabuni.pl</p>
      </div>
      <div class="col site-map">
        <p class="title">Mapa witryny:</p>
        <p><a href="/index.php">Strona główna</a></p>
        <p><a href="/pages/appetizer.php">Przystawki</a></p>
        <p><a href="/pages/main-dish.php">Dania główne</a></p>
        <p><a href="/pages/drinks.php">Koktajle i inne napoje</a></p>
      </div>
    </div>
  </footer>
</body>

</html>